...
export class TimerService {
  ...
  public getConfigs(): Observable<TimerConfig[]> {
    return this.http.get<TimerConfig[]>(`${environment.backend.protocol}${baseUrl()}/secured/v1/timer`)
  }

  public saveConfig(config: TimerConfig): Observable<never> {
    return this.http.post<void>(`${environment.backend.protocol}${baseUrl()}/secured/v1/timer`, config)
      .pipe(mergeMap(() => EMPTY))
  }
